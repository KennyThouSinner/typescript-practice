...
\`\`\` `);
    ...
    public compareArgs = (providedArg: { args: string[], argsIndex: number }, compares: { arguments: string[] }) => {

        if (compares.arguments.some(r => providedArg.args[providedArg.argsIndex].toLowerCase() === r.toLowerCase())) return true;
        return false;
